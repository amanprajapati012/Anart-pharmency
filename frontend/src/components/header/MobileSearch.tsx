  ...
  return (
    <div className="fixed inset-0 z-[3000] bg-background p-5 md:hidden overflow-y-auto">
      ...
      <div className="space-y-4">
        <div className="w-full bg-[#C9E5D7]/30 px-4 py-3 rounded-md flex justify-between items-center text-sm font-medium border border-[#0D281B]/10">
          All products
          <IconChevronDown size={16} />
        </div>

        <div className="flex border border-[#0D281B] rounded-md overflow-hidden">
          <input
            autoFocus
            value={query}
        ...
        {query && (
          <div className="rounded-xl border border-[#0D281B]/10 overflow-hidden bg-background">
            {results.length === 0 ? (
              ...
              results.map((product) => {
                ...
                return (
                  <Link
                    ...
                    className="flex items-center gap-3 p-3 border-b border-[#0D281B]/5 hover:bg-[#D4EDE0] transition"
                  >
                    <img
                      src={image}
