...
export default function ComboGallery({ combo }: any) {
  ...
  return (
    <div className="w-full max-w-full min-w-0 overflow-hidden space-y-4">
      <div className="w-full overflow-hidden rounded-[24px] md:rounded-[30px] border border-[#0D281B]/10 bg-[var(--surface)]">
        <div className="relative w-full aspect-square overflow-hidden p-2 sm:p-4 max-w-full">
          ...
          <div className="absolute top-3 left-3 sm:top-4 sm:left-4">
            <div className="flex items-center gap-2 bg-footer text-[#D19151] px-3 py-2 rounded-full text-[10px] sm:text-xs font-bold">
              <Sparkles size={13} />
              Combo Deal
            </div>
          </div>
          ...
          {images.map((img: any, index: number) => (
            <button
              ...
              className={`shrink-0 w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 rounded-2xl overflow-hidden border-2 transition-all ${
                activeIndex === index
                  ? "border-[#D19151]"
                  : "border-[#0D281B]/10 hover:border-[#0D281B]/30"
              }`}
            >
              <img
                src={getImageUrl(img)}
