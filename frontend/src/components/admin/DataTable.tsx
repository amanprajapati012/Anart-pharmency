...
export default function DataTable({
  ...
  return (
    <div className="rounded-2xl border border-[#BDE4D0] bg-white shadow-sm">

      {/* SEARCH BAR */}
      <div className="flex items-center justify-between border-b border-[#CDEADC] px-5 py-4">
        ...
        <div className="relative w-72">
          ...
          <input
            ...
            placeholder="Search..."
            className="w-full rounded-lg border pl-9 pr-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#0D281B]"
          />
        </div>
      </div>
          ...
          <tbody>
            {loading ? (
              ...
              paginatedData.map((row, index) => (
                <tr
                  key={index}
                  className="border-t border-[#CDEADC] hover:bg-[#F4FBF8]"
                >
                  {columns.map((column) => (
                    <td
                      key={column.key}
      ...
      {!loading && filteredData.length > 0 && (
        <div className="flex items-center justify-between border-t border-[#CDEADC] px-5 py-4">
